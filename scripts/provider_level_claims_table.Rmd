...
num_claims_df$cat <- ifelse((num_claims_df$n >= 1 & num_claims_df$n < 100), "1-100",
                          ifelse((num_claims_df$n >= 100 & num_claims_df$n < 200), "100-200",
                                 ifelse((num_claims_df$n >= 200 & num_claims_df$n < 300), "200-300",
                                        ifelse((num_claims_df$n >= 300 & num_claims_df$n < 400), "300-400",
                                               ifelse((num_claims_df$n >= 400 & num_claims_df$n < 500), "400-500",
                                                      ifelse((num_claims_df$n >= 500 & num_claims_df$n < 600), "500-600",
                                                             ifelse((num_claims_df$n >= 600 & num_claims_df$n < 700), "600-700",
                                                                    ifelse((num_claims_df$n >= 700 & num_claims_df$n < 800), "700-800",
                                                                           ifelse((num_claims_df$n >= 800 & num_claims_df$n < 900), "800-900",
                                                                                  ifelse((num_claims_df$n >= 900 & num_claims_df$n < 1000), "900-1000", ">1000"))))))))))

num_claims_df$cat <- factor(num_claims_df$cat, levels = c("1-100","100-200","200-300","300-400","400-500","500-600","600-700","700-800","800-900","900-1000",">1000"))
...
for(i in 1:nrow(unique_providers)){
  ...
  totalservice[i] <- as.numeric(unique_claims2[which(unique_claims2$ProviderLastName==unique_providers$ProviderLastName[i] &
                                                    unique_claims2$ProviderFirstName==unique_providers$ProviderFirstName[i]),] %>%
                               count())
  
  servicedf <- data.frame(table(unique_claims2$CDT_category[which(unique_claims2$ProviderLastName==unique_providers$ProviderLastName[i] &
                                                                        unique_claims2$ProviderFirstName==unique_providers$ProviderFirstName[i])]))
  servicedf <- servicedf[order(servicedf$Freq, decreasing = TRUE),]
  service[i] <- as.character(servicedf[1,1])
  
  servicedfp <- data.frame(prop.table(table(unique_claims2$CDT_category[which(unique_claims2$ProviderLastName==unique_providers$ProviderLastName[i] &
                                                                                    unique_claims2$ProviderFirstName==unique_providers$ProviderFirstName[i])])))
  servicedfp <- servicedfp[order(servicedfp$Freq, decreasing = TRUE),]
  servicep[i] <- as.character(paste0(round(servicedfp[1,2]*100,1),"%"))
  
  npidf <- data.frame(table(unique_claims2$SERVICING_PROVIDER_NPI[which(unique_claims2$ProviderLastName==unique_providers$ProviderLastName[i] &
                                                                        unique_claims2$ProviderFirstName==unique_providers$ProviderFirstName[i])]))
  npidf <- npidf[order(npidf$Freq, decreasing = TRUE),]
  npi[i] <- as.character(npidf[1,1])
  
  npidfp <- data.frame(prop.table(table(unique_claims2$SERVICING_PROVIDER_NPI[which(unique_claims2$ProviderLastName==unique_providers$ProviderLastName[i] &
                                                                                    unique_claims2$ProviderFirstName==unique_providers$ProviderFirstName[i])])))
  npidfp <- npidfp[order(npidfp$Freq, decreasing = TRUE),]
  npip[i] <- as.character(paste0(round(npidfp[1,2]*100,1),"%"))
...
provider_tab <- cbind.data.frame(FirstName = unique_providers$ProviderFirstName,
                                 ...
                                 MostFreqService = paste0(service, " ", "(", servicep, ")"),
                                 MostFreqNPI = paste0(npi, " ", "(", npip, ")"))

```

# Joining with all_dental_providers file to get licence info

```{r}
adp <- all_dental_providers
dim(adp) #24208

# returns string w/o leading whitespace
trim.leading <- function (x)  sub("^\\s+", "", x)
adp$PTL_P_LAST <- trim.leading(adp$PTL_P_LAST)
adp$PTL_P_FIRST <- trim.leading(adp$PTL_P_FIRST)

adp <- adp[order(adp$PTL_P_LAST, decreasing = FALSE),]

#getting licence
adp_licence <- adp %>%
  distinct(PTL_P_LAST, PTL_P_FIRST, PTL_LICENSE) 
dim(adp_licence) #7659 
adp_licence <- adp_licence[-which(is.na(adp_licence$PTL_LICENSE)),]
dim(adp_licence) #1112


licence_func <- function(first,last){
  adp_licence[grepl(as.character(last), adp_licence$PTL_P_LAST) & grepl(as.character(first), adp_licence$PTL_P_FIRST),]$PTL_LICENSE
}
licences <- mapply(licence_func, provider_tab$FirstName, provider_tab$LastName)
length(licences)
licences <- lapply(licences, unique)

number_of_unique_licences <- do.call("c",lapply(licences, length))
table(number_of_unique_licences)
#  0   1   2 
#815 387   7 
provider_tab[which(number_of_unique_licences>1),]

licences_rbind <- plyr::ldply(licences, rbind)
colnames(licences_rbind) <- paste0("License_", colnames(licences_rbind))

provider_tab <- cbind.data.frame(provider_tab,licences_rbind)
provider_tab$License_1 <- as.character(provider_tab$License_1)
provider_tab$License_2 <- as.character(provider_tab$License_2)

#investigating providers whose npi was not filled in
table(rowSums(!is.na(provider_tab[,10:11])))
provider_tab[which(rowSums(!is.na(provider_tab[,10:11]))==0),1:2]
nrow(provider_tab[which(rowSums(!is.na(provider_tab[,10:11]))==0),1:2]) #815
n=nrow(provider_tab[which(rowSums(!is.na(provider_tab[,10:11]))==0),1:2])

#fuzzy matching provider names that have missing licenses one by one
provider_tab_miss <- provider_tab[which(rowSums(!is.na(provider_tab[,10:11]))==0),]
fn <- as.character(provider_tab[which(rowSums(!is.na(provider_tab[,10:11]))==0),1])
ln <- as.character(provider_tab[which(rowSums(!is.na(provider_tab[,10:11]))==0),2])
for(i in 1:n){
  #provider last name with missing license
  x <- stringdist(ln[i], adp_licence$PTL_P_LAST)
  y <- stringdist(fn[i], adp_licence$PTL_P_FIRST)
  #adp_licence[which.min(x),]
  #adp_licence[which.min(y),]
  if(which.min(x)==which.min(y)){
    provider_tab_miss$License_1[i] <- adp_licence$PTL_LICENSE[which.min(x)]
  }
}
table(rowSums(!is.na(provider_tab_miss[,10:11])))

provider_tab_new <- rbind.data.frame(provider_tab_miss, provider_tab[-which(rowSums(!is.na(provider_tab[,10:11]))==0),])
provider_tab_new <- provider_tab_new[order(provider_tab_new$LastName, decreasing = FALSE),]
provider_tab_new$FirstName <- as.character(provider_tab_new$FirstName)
provider_tab_new$LastName <- as.character(provider_tab_new$LastName)
...
```{r}
adp_PTL_CITY_CNTY <- adp %>%
    distinct(PTL_P_FIRST, PTL_P_LAST) 

city_func <- function(first,last){
  adp_PTL_CITY_CNTY[grepl(as.character(last), adp_PTL_CITY_CNTY$PTL_P_LAST) & grepl(as.character(first), adp_PTL_CITY_CNTY$PTL_P_FIRST),]$PTL_CITY_CNTY
}
citys <- mapply(city_func, provider_tab_new$FirstName, provider_tab_new$LastName)
length(citys)

c <- unlist(lapply(citys, function(x){
  d <- data.frame(table(x))
  d <- d[order(d$Freq, decreasing = TRUE),]
  if(class(d)=="data.frame"){d <- d[1,1]; d <- as.character(d)}else{d <- NA}}))

p <- unlist(lapply(citys, function(x){
    d <- data.frame(prop.table(table(x)))
    if(nrow(d)>0){
      d <- d[order(d$Freq, decreasing = TRUE),]
      d <- round(d[1,2]*100,1)
    }else{d <- NA}}))
...
provider_tab_new$PTL_CITY_CNTY <- cp

#filling in missing
length(which(provider_tab_new$PTL_CITY_CNTY=="NA (NA%)")) #170
n=length(which(provider_tab_new$PTL_CITY_CNTY=="NA (NA%)"))

#fuzzy matching provider names that have missing licenses one by one
provider_tab_new_miss <- provider_tab_new[which(provider_tab_new$PTL_CITY_CNTY=="NA (NA%)"),]
fn <- as.character(provider_tab_new[which(provider_tab_new$PTL_CITY_CNTY=="NA (NA%)"),1])
ln <- as.character(provider_tab_new[which(provider_tab_new$PTL_CITY_CNTY=="NA (NA%)"),2])
for(i in 1:n){
  #provider last name with missing cnty
  x <- stringdist(ln[i], adp_PTL_CITY_CNTY$PTL_P_LAST)
  y <- stringdist(fn[i], adp_PTL_CITY_CNTY$PTL_P_FIRST)
  if(which.min(x)==which.min(y)){
    provider_tab_new_miss$PTL_CITY_CNTY[i] <- paste0(adp_PTL_CITY_CNTY$PTL_CITY_CNTY[which.min(x)], " ", "(100%)")
  }
}
table(provider_tab_new_miss$PTL_CITY_CNTY)

provider_tab_new2 <- rbind.data.frame(provider_tab_new_miss, provider_tab_new[-which(provider_tab_new$PTL_CITY_CNTY=="NA (NA%)"),])
provider_tab_new2 <- provider_tab_new2[order(provider_tab_new2$LastName, decreasing = FALSE),]
```

# PTL_SPC_CDE1

```{r}
adp_PTL_SPC_CDE1 <- adp %>%
    distinct(PTL_P_FIRST, PTL_P_LAST, PTL_SPC_CDE1) 
adp_PTL_SPC_CDE1 <- adp_PTL_SPC_CDE1[order(adp_PTL_SPC_CDE1$PTL_P_LAST, adp_PTL_SPC_CDE1$PTL_SPC_CDE1, decreasing = FALSE),]

spc_func <- function(first,last){
  unique(na.omit(adp_PTL_SPC_CDE1[grepl(as.character(last), adp_PTL_SPC_CDE1$PTL_P_LAST) & grepl(as.character(first), adp_PTL_SPC_CDE1$PTL_P_FIRST),]$PTL_SPC_CDE1))
}
spcs <- mapply(spc_func, provider_tab_new2$FirstName, provider_tab_new2$LastName)
length(spcs)
table(unlist(lapply(spcs, length)))

c <- unlist(lapply(spcs, function(x){
  d <- data.frame(table(x))
  d <- d[order(d$Freq, decreasing = TRUE),]
  if(class(d)=="data.frame"){d <- d[1,1]; d <- as.character(d)}else{d <- NA}}))

p <- unlist(lapply(spcs, function(x){
    d <- data.frame(prop.table(table(x)))
    if(nrow(d)>0){
      d <- d[order(d$Freq, decreasing = TRUE),]
      d <- round(d[1,2]*100,1)
...
provider_tab_new2$PTL_SPC_CDE1 <- cp

#filling in missing
length(which(provider_tab_new2$PTL_SPC_CDE1=="NA (NA%)")) #176
n=length(which(provider_tab_new2$PTL_SPC_CDE1=="NA (NA%)"))

#fuzzy matching provider names that have missing licenses one by one
provider_tab_new2_miss <- provider_tab_new2[which(provider_tab_new2$PTL_SPC_CDE1=="NA (NA%)"),]
fn <- as.character(provider_tab_new2[which(provider_tab_new2$PTL_SPC_CDE1=="NA (NA%)"),1])
ln <- as.character(provider_tab_new2[which(provider_tab_new2$PTL_SPC_CDE1=="NA (NA%)"),2])
for(i in 1:n){
  #provider last name with missing spc
  x <- stringdist(ln[i], adp_PTL_SPC_CDE1$PTL_P_LAST)
  y <- stringdist(fn[i], adp_PTL_SPC_CDE1$PTL_P_FIRST)
  if(which.min(x)==which.min(y)){
    if(!is.na(adp_PTL_SPC_CDE1$PTL_SPC_CDE1[which.min(x)])){
      provider_tab_new2_miss$PTL_SPC_CDE1[i] <- paste0(adp_PTL_SPC_CDE1$PTL_SPC_CDE1[which.min(x)], " ", "(100%)")
    }else{
      provider_tab_new2_miss$PTL_SPC_CDE1[i] <- "NA (NA%)"
    }
  }
}
table(provider_tab_new2_miss$PTL_SPC_CDE1)

provider_tab_new3 <- rbind.data.frame(provider_tab_new2_miss, provider_tab_new2[-which(provider_tab_new2$PTL_SPC_CDE1=="NA (NA%)"),])
provider_tab_new3 <- provider_tab_new3[order(provider_tab_new3$LastName, decreasing = FALSE),]
```
...
```{r}
adp_PTL_CLS_TP1 <- adp %>%
    distinct(PTL_P_FIRST, PTL_P_LAST, PTL_CLS_TP1) 
adp_PTL_CLS_TP1 <- adp_PTL_CLS_TP1[order(adp_PTL_CLS_TP1$PTL_P_LAST, adp_PTL_CLS_TP1$PTL_CLS_TP1, decreasing = FALSE),]

cls_func <- function(first,last){
  unique(na.omit(adp_PTL_CLS_TP1[grepl(as.character(last), adp_PTL_CLS_TP1$PTL_P_LAST) & grepl(as.character(first), adp_PTL_CLS_TP1$PTL_P_FIRST),]$PTL_CLS_TP1))
}
clss <- mapply(cls_func, provider_tab_new3$FirstName, provider_tab_new3$LastName)
length(clss)
table(unlist(lapply(clss, length)))

c <- unlist(lapply(clss, function(x){
  d <- data.frame(table(x))
  d <- d[order(d$Freq, decreasing = TRUE),]
  if(class(d)=="data.frame"){d <- d[1,1]; d <- as.character(d)}else{d <- NA}}))

p <- unlist(lapply(clss, function(x){
    d <- data.frame(prop.table(table(x)))
    if(nrow(d)>0){
      d <- d[order(d$Freq, decreasing = TRUE),]
      d <- round(d[1,2]*100,1)
...
provider_tab_new3$PTL_CLS_TP1 <- cp

#filling in missing
length(which(provider_tab_new3$PTL_CLS_TP1=="NA (NA%)")) #170
n=length(which(provider_tab_new3$PTL_CLS_TP1=="NA (NA%)"))

#fuzzy matching provider names that have missing licenses one by one
provider_tab_new3_miss <- provider_tab_new3[which(provider_tab_new3$PTL_CLS_TP1=="NA (NA%)"),]
fn <- as.character(provider_tab_new3[which(provider_tab_new3$PTL_CLS_TP1=="NA (NA%)"),1])
ln <- as.character(provider_tab_new3[which(provider_tab_new3$PTL_CLS_TP1=="NA (NA%)"),2])
for(i in 1:n){
  #provider last name with missing spc
  x <- stringdist(ln[i], adp_PTL_CLS_TP1$PTL_P_LAST)
  y <- stringdist(fn[i], adp_PTL_CLS_TP1$PTL_P_FIRST)
  if(which.min(x)==which.min(y)){
    if(!is.na(adp_PTL_CLS_TP1$PTL_CLS_TP1[which.min(x)])){
      provider_tab_new3_miss$PTL_CLS_TP1[i] <- paste0(adp_PTL_CLS_TP1$PTL_CLS_TP1[which.min(x)], " ", "(100%)")
    }else{
      provider_tab_new3_miss$PTL_CLS_TP1[i] <- "NA (NA%)"
    }
  }
}
table(provider_tab_new3_miss$PTL_CLS_TP1)

provider_tab_new4 <- rbind.data.frame(provider_tab_new3_miss, provider_tab_new3[-which(provider_tab_new3$PTL_CLS_TP1=="NA (NA%)"),])
provider_tab_new4 <- provider_tab_new4[order(provider_tab_new4$LastName, decreasing = FALSE),]
```
...
```{r}
adp_PTL_P_TITLE <- adp %>%
    distinct(PTL_P_FIRST, PTL_P_LAST, PTL_P_TITLE) 
adp_PTL_P_TITLE <- adp_PTL_P_TITLE[order(adp_PTL_P_TITLE$PTL_P_LAST, adp_PTL_P_TITLE$PTL_P_TITLE, decreasing = FALSE),]

title_func <- function(first,last){
  unique(na.omit(adp_PTL_P_TITLE[grepl(as.character(last), adp_PTL_P_TITLE$PTL_P_LAST) & grepl(as.character(first), adp_PTL_P_TITLE$PTL_P_FIRST),]$PTL_P_TITLE))
}
titles <- mapply(title_func, provider_tab_new3$FirstName, provider_tab_new3$LastName)
length(titles)
table(unlist(lapply(titles, length)))

c <- unlist(lapply(titles, function(x){
  d <- data.frame(table(x))
  d <- d[order(d$Freq, decreasing = TRUE),]
  if(class(d)=="data.frame"){d <- d[1,1]; d <- as.character(d)}else{d <- NA}}))

p <- unlist(lapply(titles, function(x){
    d <- data.frame(prop.table(table(x)))
    if(nrow(d)>0){
      d <- d[order(d$Freq, decreasing = TRUE),]
      d <- round(d[1,2]*100,1)
...
provider_tab_new4$PTL_P_TITLE <- cp

#filling in missing
length(which(provider_tab_new4$PTL_P_TITLE=="NA (NA%)")) #441
n=length(which(provider_tab_new4$PTL_P_TITLE=="NA (NA%)"))

#fuzzy matching provider names that have missing licenses one by one
provider_tab_new4_miss <- provider_tab_new4[which(provider_tab_new4$PTL_P_TITLE=="NA (NA%)"),]
fn <- as.character(provider_tab_new4[which(provider_tab_new4$PTL_P_TITLE=="NA (NA%)"),1])
ln <- as.character(provider_tab_new4[which(provider_tab_new4$PTL_P_TITLE=="NA (NA%)"),2])
for(i in 1:n){
  #provider last name with missing spc
  x <- stringdist(ln[i], adp_PTL_P_TITLE$PTL_P_LAST)
  y <- stringdist(fn[i], adp_PTL_P_TITLE$PTL_P_FIRST)
  if(which.min(x)==which.min(y)){
    if(!is.na(adp_PTL_P_TITLE$PTL_P_TITLE[which.min(x)])){
      provider_tab_new4_miss$PTL_P_TITLE[i] <- paste0(adp_PTL_P_TITLE$PTL_P_TITLE[which.min(x)], " ", "(100%)")
    }else{
      provider_tab_new4_miss$PTL_P_TITLE[i] <- "NA (NA%)"
    }
  }
}
table(provider_tab_new4_miss$PTL_P_TITLE)

provider_tab_new5 <- rbind.data.frame(provider_tab_new4_miss, provider_tab_new4[-which(provider_tab_new4$PTL_P_TITLE=="NA (NA%)"),])
provider_tab_new5 <- provider_tab_new5[order(provider_tab_new5$LastName, decreasing = FALSE),]
```

# Saving file

```{r}
write.csv(provider_tab_new5, "C:/Users/stili/Downloads/providers_file.csv")
```

# Fix mismatched provider; TRAN, AN should be TRAN, DAN with 1 lencense 442000275, the other lincense (401410626) belongs to TRAN, MYVAN

```{r}
provider_file <- read.csv("C:/Users/stili/Downloads/providers_file.csv", header = T)

provider_file$License_2[which(provider_file$FirstName=="AN" & provider_file$LastName=="TRAN")] <- NA
```

# Resave file

```{r}
write.csv(provider_file, "C:/Users/stili/Downloads/providers_file.csv")
```
