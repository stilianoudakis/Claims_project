...
#getting PTL_P_TITLE, PTL_CLS_TP1, PTL_SPC_CDE1, PTL_CITY_CNTY 
n=nrow(provider_tab_new)
##PTL_CITY_CNTY
adp_PTL_CITY_CNTY <- adp %>%
    distinct(PTL_P_FIRST, PTL_P_LAST, PTL_CITY_CNTY) 
provider_tab_new$PTL_CITY_CNTY <- NA
city_list <- list()
for(i in 1:n){
  x <- stringdist(provider_tab_new$LastName[i], adp_PTL_CITY_CNTY$PTL_P_LAST)
  y <- stringdist(provider_tab_new$FirstName[i], adp_PTL_CITY_CNTY$PTL_P_FIRST)
  if(which.min(x)==which.min(y)){
    city_list[i] <- unique(adp_PTL_CITY_CNTY$PTL_CITY_CNTY[x==min(x)])
  }
}
